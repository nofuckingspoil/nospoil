// nsData.js — converts production DATA format → NS_DATA format expected by React components
  ...
  sport.competitions.forEach(comp => {
    ...
    stages[comp.id] = sorted.map(s => {
      ...
      return {
        ...
        // available/videoId/latest are injected at runtime from Supabase
        available: false,
        videoId:   null,
        latest:    false,
      };
...
export const NS_DATA = { sports, competitions, stages };

export function getNsData() {
  return NS_DATA;
}
